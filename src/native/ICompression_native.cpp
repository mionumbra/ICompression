...
static DataStream entries_to_array_stream(const std::vector<ArchiveEntry>& entries)
{
...
static std::string decompress_raw(std::string_view data, CompressionFormat format)
{
    (void)format;
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK)
    {
        ...
        if (read < 0)
        {
            LOG_ERROR("ic_decompress: archive_read_data failed: %s", archive_error_string(a));
...
std::string ic_decompress(std::string_view data, CompressionFormat format)
{
    ...
    return decompress_raw(compressed, format);
}
...
bool ic_decompress_file(std::string_view src, std::string_view dst, CompressionFormat format)
{
    ...
    std::string decompressed = decompress_raw(file_data, format);
...
CompressResult ic_decompress_buf(GMBuffer input, GMBuffer output, CompressionFormat format)
{
    ...
    std::string_view sv(input_data, input_len);
    std::string decompressed = decompress_raw(sv, format);

    if (decompressed.empty() && input_len > 0)
    {
        result.success = false;
        result.original_size = static_cast<int64_t>(input_len);
        result.compressed_size = 0;
...
DataStream ic_list(std::string_view archive)
{
    struct archive* a = archive_read_new();
    archive_read_support_filter_all(a);
    archive_read_support_format_all(a);
    ...
    if (OPEN_ARCHIVE_READ(a, archive, 10240) != ARCHIVE_OK)
    {
        ...
        archive_read_free(a);
        return entries_to_array_stream({});
    }
    ...
    return entries_to_array_stream(entries);
}
...
ExtractResult ic_extract(std::string_view archive, std::string_view output_dir)
{
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK)
    {
        ...
        {
            archive_read_data_skip(a);
            result.error_message = "Archive contains an unsafe entry path: " + entry_path;
            result.success = false;
            archive_read_close(a);
            archive_read_free(a);
...
std::string ic_extract_mem(std::string_view archive, std::string_view entry)
{
    ...
    while (archive_read_next_header(a, &ae) == ARCHIVE_OK)
    {
        ...
        if (read < 0)
        {
            LOG_ERROR("ic_extract_mem: archive_read_data failed: %s", archive_error_string(a));
            result.clear();
        }
        break;
    }
